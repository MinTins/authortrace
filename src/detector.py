...
англійською, і вже над перекладом обчислюються ознаки. Результат
включає інформацію про мову оригіналу та виконаний переклад.
"""
...
class AuthorTraceDetector:
    ...
    def analyze(self, text, threshold=0.5, translate=False):
        ...
        # Посегментний аналіз виконується над тим самим текстом, що йде
        # у глобальну модель, — це забезпечує узгодженість між загальним
        # вердиктом і розбивкою по сегментах.
